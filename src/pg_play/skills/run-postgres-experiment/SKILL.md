...
description: Plan and run reproducible PostgreSQL stand, workload, and diagnostic flows through the pg_play MCP server. Use when a user asks to create or rebuild a PostgreSQL stand, apply a pg_configurator candidate, emulate backend activity with pg_workload, collect a pg_diag report, repeat an experiment with changed parameters, or inspect experiment status.
---
...
3. Call `plan_experiment`. Present the required stand action, configuration artifact hash, selected workload profiles, diagnostic mode, warnings, and `plan_hash`.
...
8. Report artifact paths and distinguish `succeeded` from `partial`. A partial pg_diag report remains inspectable.
